...
public interface AsrKpiRepo extends JpaRepository<AsrKpi, Long> {
    ...
    @Query(value = "SELECT kpi.asr, kpi.ner, kpi.date, kpi.object_instance as objectInstance  from tb_asr_kpi as kpi GROUP BY kpi.date", nativeQuery = true)
    ...
    List<AsrKpiDto> findAsrKpiByObjectInstance(@Param("objectInstance") String objectInstance);
}
